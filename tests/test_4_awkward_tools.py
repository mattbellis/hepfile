...
def test_hepfile_to_awkward():
    ...
    for group in hepfile["_GROUPS_"].keys():
        for key in hepfile["_GROUPS_"][group]:
            if f"{group}/{key}" == hepfile["_MAP_DATASETS_TO_COUNTERS_"][group]:
                continue
    ...
    awk_subset = hf.awkward_tools.hepfile_to_awkward(
        hepfile, groups="jet", datasets=["jet/e", "jet/px"]
    )
...
def test_awkward_to_hepfile():
    ...
    newdata = hf.awkward_tools.awkward_to_hepfile(
        awk, outfile="awk-test.h5", write_hepfile=True, verbose=True
    )
    ...
    assert ak.all(
        ak.sort(data["_GROUPS_"]["jet"]) == ak.sort(newdata["_GROUPS_"]["jet"])
    )
...
def test_get_awkward_type():
    ...
    with pytest.raises(hf.errors.InputError):
        t = hf.awkward_tools._get_awkward_type(
            ak.Array([np.array(["1"]), np.array([1, 2])])
        )
